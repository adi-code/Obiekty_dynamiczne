...
/* Routine to perform non-dominated sorting (high performance code, uses a customized doubly linked list, so that all the *required* operations are O(1).) */
void OmniOptOptimizer::fill_nondominated_sort (OmniOptOptimizer::population *mixed_pop, OmniOptOptimizer::population *new_pop)
{
    int flag;
    int i, j;
    int end;
    ...
    do
    {
        ...
        do
        {
            ...
            do
            {
                ...
                flag = check_loose_dominance (&(mixed_pop->ind[temp1->index]), &(mixed_pop->ind[temp2->index]));
                if (flag == 1)
                {
...
void OmniOptOptimizer::crowding_fill (OmniOptOptimizer::population *mixed_pop, OmniOptOptimizer::population *new_pop, int count, int front_size, list *elite)
{
    int *dist;
    list *temp;
    int i, j;
